...
class egreled_withNE_dataset(Dataset):
    def __init__(
        self,
        dataset_root,
        height,
        ...
        self.H = height
        self.W = width
        self.noraml_img_folder = os.path.join(dataset_root, seq_name, "gt_processed")
        ...
        self.low_img_folder = os.path.join(dataset_root, seq_name, "blur_processed")
        ...
        self.events_folder = os.path.join(dataset_root, seq_name, "events")
        self.events_list_all = os.listdir(self.events_folder)
        self.events_list = sorted(list(filter(lambda x: 'npz' in x, self.events_list_all))) 
        ...
        if self.is_split_event:
            ...
            self.low_ev_list = sorted(list(filter(lambda x: 'npz' in x, self.events_list_all)))
        else:
            # use the whole event file
            self.normal_event_file = os.path.join(
            dataset_root, seq_name, "gt_processed", "normalight_event.npz"
        )
            self.low_event_file = os.path.join(
            dataset_root, seq_name, "blur_processed", "lowlight_event.npz"
        )
    ...
    def __getitem__(self, index):
        ...
        if (self.events is None) & (self.is_split_event == False) :
            ...
            events = np.load(os.path.join(self.events_folder, self.low_ev_list[index]))
        else:
        ...
        sample = {
            "lowligt_image": torch.pow(crop_img_list[0]/255,0.6),
...
def get_egreled_withNE_dataset(
    dataset_root,
