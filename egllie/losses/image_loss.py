...
class PerceptualLoss(_Loss):
    ...
    def forward(self, batch):
        pred, gt = batch["pred"], batch["gt"]
        self.lpips_loss.to(pred.device)
        loss = self.lpips_loss(pred, gt, normalize=True).mean()
        return loss
...
class SSIMLoss(_Loss):
    def __init__(self, window_size=9, size_average=True, value_range=1.0):
        super().__init__()
        self.window_size = window_size
        self.size_average = size_average
        self.value_range = value_range
        self.ssim_fn = SSIM(window_size=window_size,
                            size_average=size_average,
                            value_range=value_range)

    def forward(self, batch):
        pred, gt = batch["pred"], batch["gt"]
        ssim_val = self.ssim_fn(pred, gt)

        return 1.0 - ssim_val

class MSSSIMLoss(nn.Module):
    def __init__(self, value_range=1.0, size_average=True):
    ...
    def forward(self, batch):
        pred, gt = batch["pred"], batch["gt"]
        ms_ssim_val = ms_ssim(pred, gt, data_range=self.value_range, size_average=self.size_average)
